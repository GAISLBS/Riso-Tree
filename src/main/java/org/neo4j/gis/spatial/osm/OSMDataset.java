...
public class OSMDataset
    implements SpatialDataset, Iterable<OSMDataset.Way>, Iterator<OSMDataset.Way> {
  ...
  public OSMDataset(SpatialDatabaseService spatialDatabase, OSMLayer osmLayer, Node layerNode,
    ...
    try (Transaction tx = spatialDatabase.getDatabase().beginTx()) {
      ...
      Relationship rel =
          layerNode.getSingleRelationship(SpatialRelationshipTypes.LAYERS, Direction.INCOMING);
      if (rel == null) {
        ...
        if (!node.equals(datasetNode)) {
          throw new SpatialDatabaseException(
              "Layer '" + osmLayer + "' already belongs to another dataset: " + node);
        }
      }
      tx.success();
    }
  ...
  /**
   * This method is used to construct the dataset when only the layer node is known, and the dataset
   * node needs to be searched for.
   * 
   * @param spatialDatabase
   * @param osmLayer
   * @param layerNode
  ...
  public OSMDataset(SpatialDatabaseService spatialDatabase, OSMLayer osmLayer, Node layerNode) {
    try (Transaction tx = layerNode.getGraphDatabase().beginTx()) {
      ...
      if (rel == null) {
        throw new SpatialDatabaseException(
            "Layer '" + osmLayer + "' does not have an associated dataset");
      } else {
  ...
  public Iterable<Node> getAllUserNodes() {
    TraversalDescription td = datasetNode.getGraphDatabase().traversalDescription().depthFirst()
  ...
  public Way getWayFrom(Node osmNodeOrWayNodeOrGeomNode) {
    TraversalDescription td = datasetNode.getGraphDatabase().traversalDescription().depthFirst()
        ...
        .relationships(OSMRelation.GEOM, Direction.INCOMING).evaluator(new Evaluator() {
          @Override
