...
public class OwnMethods {
  ...
  /**
   * Convert the one-predicate query graph to two-predicate. The first spatial vertex will be set to
   * be true The query graph is for LAGAQ-join input.
   * 
   * @param query_Graph
   */
  ...
  /**
   * Convert the original single directional graph to a bidirectional graph where each edge will
   * stored in both start and end node's adjacent list.
   * 
   * @param graph
   * @return
   */
  public static ArrayList<TreeSet<Integer>> singleDirectionalToBidirectionalGraph(
  ...
  /**
   * Generate Query rectangle spatial selectivity considering space area center location space
   * oriented
   * 
   * @param rect_size_x
   * @param rect_size_y
   * @param total_range
   */
  public static MyRectangle GenerateQueryRectangle(Random r, double rect_size_x, double rect_size_y,
  ...
  /**
   * Generic function for generating id of spatial vertices. This will lead to centers generated
   * being data-density-oriented, which means that dense area will have more query rectangles.
   * 
   * @param entities
   * @param center_id_path
   * @param experimentCount
   */
  public static void generateQueryRectangleCenterID(ArrayList<Entity> entities,
      String center_id_path, int experimentCount) {
    try {
      ...
      do {
        ...
        if (!((Entity) entities.get((int) center_id)).IsSpatial || !center_ids_set.add(center_id))
          continue;
  ...
  public static DefaultMutableTreeNode GetExecutionPlanTree(ExecutionPlanDescription plan,
      int node_count) {
    try {
      DefaultMutableTreeNode root = null;
      String plan_name = plan.getName();
      while (plan_name.equals("NodeByLabelScan") == false
          && plan_name.equals("Expand(All)") == false
  ...
  public static void Search(DefaultMutableTreeNode root, ExecutionPlanDescription plan,
      int node_count) {
    try {
...
//      query_Graph.label_list_string[i] = labelStringMap[label];
  ...
  public static boolean ReachabilityQuery(ArrayList<ArrayList<Integer>> graph,
    ...
    while (!queue.isEmpty()) {
      ...
      if (entities.get((int) id).IsSpatial && OwnMethods
          .Location_In_Rect(entities.get((int) id).lat, entities.get((int) id).lon, query_rect)) {
        return true;
      }
      int i2 = 0;
